...
int		view_plane_camera(t_univers *v)
{
	...
	v->camera->upvect = ft_creat_vector((float)1, (float)0, (float)0);
	v->camera->rightvect = ft_creat_vector((float)0, (float)1,
			(float)0);
	printf("%f\n",v->camera->upvect.x_pos);
	printf("%f\n",v->camera->upvect.y_pos);
	printf("%f\n",v->camera->upvect.z_pos);
	v->view_up_left  = ft_add_vector(v->camera->position
						, ft_float_by_vector((float)CAMERA_DIST_V
							, v->camera->direction))
...
t_vector		ft_float_by_vector(float a, t_vector v2)
{
	t_vector	new_v;
